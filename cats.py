...
def accuracy(typed, reference):
    ...
    # BEGIN PROBLEM 3
    words = typed_words[:len(reference_words)]
    correct = 0
    for i, j in zip(words, reference_words):
        if i == j:
            correct += 1
    if (len(typed_words) == 0 and len(reference_words) != 0) or (len(typed_words) != 0 and len(reference_words) == 0):
        return 0.0
    return 100.0 if (len(typed_words) == 0 and len(reference_words) == 0) else (correct / len(typed_words)) * 100 
    # END PROBLEM 3
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        return typed_word
    low_diff = min(valid_words, key = lambda i: diff_function(typed_word, i, limit))
    word = diff_function(typed_word, low_diff, limit)
    if word <= limit:
        return low_diff
    else:
        return typed_word
...
def minimum_mewtations(start, goal, limit):
    ...
    if len(start) == 0 or len(goal) == 0:  # Fill in the condition
        ...
        # BEGIN
        return add if add < min(remove, substitute) else min(remove, substitute)
        # END
...
def time_per_word(words, times_per_player):
    ...
    for i in range(len(times_per_player)):
        ...
        for j in range(len(times_per_player[i]) - 1):
            new_time.append(times_per_player[i][j + 1] - times_per_player[i][j])
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    fast_times = [[] for x in player_indices]
    for i in word_indices:
        times = []
        for j in player_indices:
            times.append(get_times(match)[j][i])
        index = min(player_indices, key = lambda f: times[f])
        fast_times[index].append(word_at(match, i))
    return fast_times
    # END PROBLEM 10
